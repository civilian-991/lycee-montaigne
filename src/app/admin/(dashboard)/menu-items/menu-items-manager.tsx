...
export function MenuItemsManager({ initialItems }: { initialItems: MenuItem[] }) {
  ...
  async function handleAddChild(e: React.FormEvent<HTMLFormElement>, parentId: string) {
    ...
    setLoading(true);
    const form = new FormData(e.currentTarget);
    const parent = initialItems.find((i) => i.id === parentId);
    await fetch("/api/admin/menu-items", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    <div>
      <div className="space-y-2">
        {initialItems.map((item) => (
          <div key={item.id} className="rounded-xl border border-border bg-white">
            {/* Top-level item header */}
            <div className="p-4">
              {editingId === item.id ? (
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-text">Libellé *</label>
                    <input
                      required
              ...
              <div className="border-t border-border bg-background-alt/30 px-4 py-2">
                <div className="space-y-1">
                  {item.children.map((child) => (
                    <div key={child.id} className="rounded-lg px-3 py-2.5">
                      {editingId === child.id ? (
                        <div className="space-y-3">
                          <div>
                            <label className="block text-sm font-medium text-text">Libellé *</label>
      ...
      {initialItems.length === 0 && !showTopForm && (
      ...
      <DeleteModal
        ...
        description={`L'élément « ${deleteLabel} » et tous ses sous-éléments seront définitivement supprimés. Voulez-vous continuer ?`}
