      ...
      <div className={cn(
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2 text-sm">
          <div className="flex items-center gap-4">
            ...
            <Link href="/anciens" className="hover:text-secondary-light transition-colors">
              Anciens élèves
            </Link>
          </div>
          <div className="hidden items-center gap-3 sm:flex">
            <a href="https://www.facebook.com/LyceeMontaigneBeitChabab" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="hover:text-secondary-light">
            ...
            <a href="https://www.linkedin.com/school/lyc%C3%A9e-montaigne-beit-chabab/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="hover:text-secondary-light">
            ...
            <a href="https://www.instagram.com/lyceemontaignebeitchabab/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="hover:text-secondary-light">
      ...
      <nav className="mx-auto max-w-7xl px-4" aria-label="Navigation principale">
        <div className="flex h-16 items-center justify-between">
            ...
            {navigation.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className={cn(
                    "flex items-center gap-1 rounded-md px-2.5 py-2 text-sm font-medium text-text transition-colors hover:bg-background-alt hover:text-primary",
                    openDropdown === item.label && "bg-background-alt text-primary"
                  )}
                >
                  {item.label}
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/contact"
              className="hidden rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-light md:block"
            >
              Contactez-nous
            </Link>
            <button
        ...
        <nav className="overflow-y-auto p-4" style={{ maxHeight: "calc(100vh - 65px)" }}>
            ...
            <div key={item.label} className="border-b border-border/50 last:border-0">
              {item.children ? (
                <details className="group">
                  <summary className="flex cursor-pointer list-none items-center justify-between py-3 text-sm font-medium text-text">
                    {item.label}
                ...
                <Link href={item.href} onClick={() => setMobileOpen(false)} className="block py-3 text-sm font-medium text-text">
                  {item.label}
                </Link>
              )}
            </div>
          ...
          <div className="mt-4 space-y-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 rounded-lg bg-secondary px-4 py-2.5 text-sm font-medium text-white"
            >
              Pronote <ExternalLink className="h-3.5 w-3.5" />
            </a>
            <Link href="/contact" onClick={() => setMobileOpen(false)} className="block rounded-lg bg-primary px-4 py-2.5 text-center text-sm font-medium text-white">
              Contactez-nous
            </Link>
          </div>
        </nav>
